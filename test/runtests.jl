...
@testset verbose=true "StochasticFrontiers.jl Tests" begin
    ...
    @testset verbose=true "Example 1.4: Truncated Normal, BC1995" begin
        res = sfmodel_fit(
            ...
            options = sfopt(verbose = false),
            init    = sfinit(μ = fill(0.1, 4), log_σᵥ² = -0.1)
        )
    ...
    @testset verbose=true "Example 6  : panel FE model of Wang and Ho, (Half Normal)" begin
        res = sfmodel_fit(
            ...
            options = sfopt(
                warmstart_maxIT  = 100,
                warmstart_solver = Optim.NewtonTrustRegion(), 
    ...
    @testset verbose=true "Example 7  : panel FE model of Wang and Ho (Truncated Normal)" begin
        res = sfmodel_fit(
            ...
            options = sfopt(warmstart_maxIT = 200, verbose = false)
        )

        marginal, marginal_mean = sfmarginal(res)
        std_ci, sim_res         = sfmarginal_bootstrap(res, R = 10, seed = 123)
        ...
        @test std_ci[1, 1]              ≈ 0.03323            atol = 1e-5 
        # @test std_ci[1, 1]              ≈ 0.03087            atol = 1e-5
    end
    ...
    @testset verbose=true "Example 12 : panel correlated RE model with serial correlated error" begin
        ...
        @test mean(sf_inefficiency(res)) ≈ 0.0446956          atol = 1e-5
        @test mean(sf_efficiency(res))   ≈ 0.9569412          atol = 1e-5
    end
end  # end of the test
